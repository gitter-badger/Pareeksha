...
var _om_gbls = {
	...
	apl : "", split : "", addrsid : "", mscooks : "", muid : "no_muid", anid : "no_anid", products : "",
...
_om_gbls.addrsid= function(matchType,str,rsid) {
	...
	} else if (matchType=='p' && _om_gbls.path.indexOf(str)==0) { 
		_om_gbls.s_account = _om_gbls.apl(_om_gbls.s_account,rsid,",",2);
...
function getOmniPath(pval,skipBeginning,skipEnd) {
	if (skipEnd == 1) pval += "foo"; // handle URLs ending in just /
...
if (getOmniMetaTagData("t_omni_tracksearchbox")==1) {
	// target the first form on the page
	document.forms[0].onsubmit = function() {
		// look for goButton and q on the form
		var gb = document.getElementById('goButton'), q_string = document.forms[0].q.value, search_url =  document.forms[0].action; 
		if (gb && q_string && search_url) {
			var oldlinktrackvars = s.linkTrackVars;
			s.linkTrackVars="eVar25,prop12,eVar12,prop13,eVar13,prop14,eVar14,prop15,eVar15,prop16,eVar16,prop17,eVar17,events";
			s.linkTrackEvents="event2"; s.events="event2";
			s.eVar12=s.prop12="";
			s.eVar13=s.prop13=s.eVar14=s.prop14=s.eVar15=s.prop15=s.eVar16=s.prop16="searchbox";
			s.eVar17=s.prop17=s.pageName;
			s.eVar25=q_string;
			s.tl(this,'o','searchbox: '+s.prop17);
			s.eVar25=s.prop12=s.eVar12=s.prop13=s.eVar13=s.prop14=s.eVar14=s.prop15=s.eVar15=s.prop16=s.eVar16=s.prop17=s.eVar17=s.events="";
...
/*
omni_stripQS: strip out query string and protocol from link param
*/
...
var omniPCreferral = false;
function omniPCPartnerReferral(that, href, cm, lt, successevent) {
	omniPCreferral = true; // set flag so that a second Atlas tracking call won't be made
	s.linkTrackEvents = "event4";
	s.linkTrackVars="prop12,prop13,prop14,prop15,prop16,prop17,prop45,eVar13,eVar15,eVar16,eVar17,eVar35,eVar36,events";
	s.prop12 = href;
	s.prop13 = s.eVar13 = cm;
	s.prop15 = s.eVar15 = "pc: " + lt + ": " + href;
	s.prop16 = s.eVar16 = "pc: " + lt;
	s.prop17 = s.eVar17 = s.pageName;
	s.events = (successevent) ? "event4" : "";
	s.eVar35 = s.prop16;
	s.eVar36 = "y";
}


function omniHeroTracking(obj,module,heroName,destURL,type,altText,evts) {
omniModTracking(obj,module,heroName,destURL,type,altText,evts);
}


try { if(extraevars.indexOf("eVar")>-1) _om_gbls.extraevars = extraevars; } catch(e) {}

function omniModTracking(obj,mod1,mod2,destURL,type,altText,evts) {
	omniProdTracking2(obj,mod1,mod2,destURL,type,altText,evts,'')
}

function omniProdTracking2(obj,mod1,mod2,destURL,type,altText,evts,prod) {
var oldltvars=s.linkTrackVars; var oldltevts=s.linkTrackEvents;
s.linkTrackVars="prop12,eVar12,prop13,eVar13,prop14,eVar14,prop15,eVar15,prop16,eVar16,prop17,eVar17,events";
if (prod) { 
...
s.events= (type>0) ? "event2" : "event16";
if(evts && (evts.indexOf('event')>-1 || evts.indexOf('sc')>-1)){s.linkTrackEvents+=","+evts;s.events+=","+evts;}
destURL = omni_StripProtocol(destURL.toLowerCase());
if(destURL.indexOf('atdmt.com')>-1) { // strip out query string and protocol for query string
	destURL = omni_StripQS(destURL);
}
s.eVar12=s.prop12=destURL;
var module=(mod2) ? mod1+":"+mod2 : mod1;
s.eVar13=s.prop13=mod1;
s.eVar14=s.prop14=module;
s.eVar15=s.prop15=module+": "+destURL;
s.eVar16=s.prop16=module+": "+altText;
s.eVar17=s.prop17=s.pageName;
...
//test and target piggy back tracking:
if (typeof applyTTTracking != 'undefined' && type != 0) {
	if ((applyTTTracking == true) && (destURL.toLowerCase().indexOf("automated") == -1)) {
		mboxUpdate('usWinGlobalTrack', "module=" + mod1, "linkText=" + altText, "mod2=" + module, "pcRetailerScenario=" + (typeof pcRetailerScenario != "undefined" ? pcRetailerScenario : "null"), "evts=" + s.events);
	}
}

s.linkTrackEvents=oldltevts;s.linkTrackVars=oldltvars;
// clear variables
s.prop12=s.eVar12=s.prop13=s.eVar13=s.prop14=s.eVar14=s.prop15=s.eVar15= s.prop16=s.eVar16=s.prop17=s.eVar17=s.events="";
}


function omniAddPVEvent() {
     if (!s.linkType) { // if not s.tl()
		 s.eVar17 = s.pageName;  // added 2/6/09
		 if (_om_gbls.products) {
			s.products = _om_gbls.products;
			s.events = _om_gbls.apl(s.events,"prodView",",",2);
		 }
		 s.events = _om_gbls.apl(s.events,"event1",",",2);
     }
}
	...
	_om_gbls.s_account = s_account;
	
	_om_gbls.extraRsids = getOmniMetaTagData("t_omni_rsids");
	_om_gbls.products = getOmniMetaTagData("t_omni_product");
	...
	_om_gbls.addrsid('p','/windows/possibilities/','mswindowsftp');
	_om_gbls.addrsid('p','/windows/windows-vista/','mswindowsvista');
	_om_gbls.addrsid('p','/windows/internet-explorer/','mswindowsie');
	_om_gbls.addrsid('p','/windows/products/winfamily/ie/','mswindowsie');
	_om_gbls.addrsid('p','/windows/downloads/ie/','mswindowsie');
	_om_gbls.addrsid('p','/windows/pc-scout/','mswindowsbuy');
	_om_gbls.addrsid('p','/windows/downloads/','mswindowsdl');
	_om_gbls.addrsid('p','/windowsxp/downloads/','mswindowsdl');
	_om_gbls.addrsid('p','/windowsxp/downloads/','mswindowsxpdl');
	_om_gbls.addrsid('p','/windows/windows-vista/mojave/','mswindowsmoj');
	_om_gbls.addrsid('p','/windows/windows-7/','mswindowswin7');
	_om_gbls.addrsid('p','/windows/enterprise/','mswindowsent');
	_om_gbls.addrsid('p','/windows/business/','mswindowsbus');
	_om_gbls.addrsid('p','/windows/compatibility/windows-7/','mswindowswin7cc');
	_om_gbls.addrsid('p','/windows/windows-media-center/','mswindowsmediacent');
	_om_gbls.addrsid('p','/windows/products/winfamily/mediacenter/','mswindowsmediacent');
	_om_gbls.addrsid('p','/windows/windowsintune/','mswindowsintune');	
	_om_gbls.addrsid('p','/windows/cloud/','mswindowscloud');	
	_om_gbls.addrsid('p','/windows/explore/','mswindowsexplore');		
	if (_om_gbls.path.indexOf('/windows/offers/ms-store-')==-1) { // added 6/21/10
		_om_gbls.addrsid('p','/windows/buy/offers/','mswindowsbuyoffers');
		_om_gbls.addrsid('p','/windows/offers/','mswindowsbuyoffers');
		_om_gbls.addrsid('p','/windows/buy/','mswindowsbuy');
		_om_gbls.addrsid('p','/windows/buy/','mswindowsextbuy');
		_om_gbls.addrsid('p','/windows/offers/','mswindowsbuy');
	}

if (typeof (_mCurrentCobrand) != 'undefined') {
	_om_gbls.s_account = _om_gbls.apl(_om_gbls.s_account,"mswindowsconcierge",",",2);
	if (_mCurrentCobrand=="default") { 
		_om_gbls.s_account = _om_gbls.apl(_om_gbls.s_account,"mswindowsscoutdef",",",2);
	} else if (_mCurrentCobrand=="cloud") { 
		_om_gbls.s_account = _om_gbls.apl(_om_gbls.s_account,"mswindowsscoutcloud",",",2);
	} else if (_mCurrentCobrand=="guided-tour") {
		_om_gbls.s_account = _om_gbls.apl(_om_gbls.s_account,"mswindowsscoutguided",",",2);
	} else if (_mCurrentCobrand=="student") {
		_om_gbls.s_account = _om_gbls.apl(_om_gbls.s_account,"mswindowsscoutstdnt",",",2);
	} else if (_mCurrentCobrand=="amd") {
		_om_gbls.s_account = _om_gbls.apl(_om_gbls.s_account,"mswindowsscoutamd",",",2);
	}
}

_om_gbls.host = window.location.hostname.toLowerCase();
if (_om_gbls.host != "www.microsoft.com" && _om_gbls.host.indexOf("lifewithoutwalls.com") == -1 && _om_gbls.host.indexOf("windows-pc-scout.jellyvision-conversation.com") == -1 && _om_gbls.host.indexOf("wlsguidedtour.jellyvision.com") == -1 ) {
...
try { if (s.c_r("ANON")) _om_gbls.anid = "yes_anon"; } catch (err) {}
try { if (s.c_r("MUID")) _om_gbls.muid = "yes_muid"; } catch (err) {}
s.prop50 = _om_gbls.anid + "," + _om_gbls.muid;
...
function s_doPlugins(s) {
    ...
    s.tnt=s.trackTNT(); // added 6/1/09

	s.prop26=s.getAndPersistValue(s.tnt,'s_tntpage_persist',0);  // added 8/11/09
    s.prop26=s.prop26?s.prop26+":"+s.pageName:'';
	...
	if(s.eVar13) {
		s.eVar22 = s.pageName +": "+ s.eVar16.substring(0,120);
		s.prop22 = "D=v22";
		s.eVar23 = s.pageName +": "+ s.eVar15.substring(0,120);
		s.prop23 = "D=v23";
		s.linkTrackVars=s.apl(s.linkTrackVars,"prop22,prop23,eVar22,eVar23",",",2);	
	}
	...
	s.campaign=s.eVar26=s.eVar18=s.getValOnce(s.campaign,"s_campaign",0)
	...
	/* SEM id */
	if(!s.eVar30)
		s.eVar30=s.getQueryParam('semid');
	s.eVar30=s.getValOnce(s.eVar30,"s_eVar30",0)
	
	/* Plugin Example: detectSilverlight v0.1 	*/
	
	s.detectSilverlight('prop10', 'svc', '');
	...
	if (url.indexOf('atdmt.com') > -1) {
		var tpath = s.p_gh_windows_new().pathname;
		var pathArray = tpath.split('/');
		var tagname="";
		var prefix="Atlas: ";
		//var lt = s.p_gh_windows_new().innerHTML;
		if(pathArray.length > 1) tagname=pathArray[1];
		s.eVar35=tagname;
		s.prop45=tpath;
		if (!s.eVar36) s.eVar36 = (tagname.indexOf("mrtwxp_winftp") > -1 || tagname.indexOf("VistaCPCBuyNow") > -1) ? "y" : "n";
		if (!omniPCreferral) {
	...
	if (url.indexOf('www.microsoft.com/downloads/details.aspx') > -1) { 
		var lnk = s.p_gh_windows_new();
		var lt = lnk.innerHTML;
		var page = new PageQuery(lnk.search); 
		var fid = unescape(page.getValue("FamilyID")); 
		...
		s.prop19 = s.eVar19 = s.pageName + ": dl: " + lt;
		s.prop20 = s.eVar20 = s.pageName + ": dl: " + fid;
	}
	
	var uastring = navigator.userAgent.toLowerCase();
	s.prop47 = (uastring.indexOf('win64\; ia64') > -1 || uastring.indexOf('win64\; x64') > -1 || uastring.indexOf('wow64') > -1) ?
		"64 bit" : "other";	
	s.eVar47="D=User-Agent";
	

	omniPCreferral = false;
...
/*
* TNT Integration Plugin v1.0,  added 6/1/09
*/
